...
class GymSnakeEnv(gym.Env):
    ...
    def __init__(self, frame_rows=12, frame_cols=12, gui=False):
        ...
        # SB3 CNNs expect 3D spaces (Channels, Height, Width). 
        # The FrameStack wrapper will turn this into (4, 12, 12)
    ...
    def _get_obs(self):
        obs = np.zeros((self.frame_rows, self.frame_cols), dtype=np.float32)
        
        for point in self.game.snake[1:]:
...
def snake_make_env(frame_rows=12, frame_cols=12, stack_size=4, gui=False):
